...
getmiRNA <- function(sp = "mmu") {
  sp <- tolower(sp)
  link <- paste0("http://www.mirbase.org/download/", sp, ".gff3")
  ...
  miRNA <- rtracklayer::import("http://www.mirbase.org/download/CURRENT/mature.fa",
    format = "fasta"
  )
  ...
  miRNA_h <- rtracklayer::import("http://www.mirbase.org/download/CURRENT/hairpin.fa",
    format = "fasta"
  )
